...
class EticController extends Controller {
    ...
    public function urunSil(Request $request,$id){
        $em = $this->getDoctrine()->getManager();
        $img = $em->getRepository(Images::class)->findBy(['product' => $id]);
    ...
    /**
     * @Route("/bilgilendirme", name="etic_bilgilendirme")
     */
    public function bilgilendirme(){
        $users = $this->getDoctrine()->getRepository(User::class)->findAll();
        return $this->render($this->TempDir.'bilgilendirme.html.twig', array('users' => $users,'title' => 'Bilgilendirme'));
    }
    ...
    public function sendmail(Request $request, \Swift_Mailer $mailer) {
        $ids = $request->request->get('emails');
        $template = $request->request->get('template');
        $user = $this->getDoctrine()->getRepository(User::class)->findBy(
            ['id' => $ids]
        );
        $emails = array_map(function ($x){return $x->getEmail();}, $user);
        $message = (new \Swift_Message('Halo'))
            ->setFrom('test@test.co')
            ->setTo($emails)
            ->setBody(
        ...
        return $this->redirectToRoute('etic_bilgilendirme');
    }
